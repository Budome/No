...
void roaring_bitmap_get_each_container_type(const roaring_bitmap_t *ra, 
    std::map<int, uint64_t>& res_type, std::vector<int>& res_num){
    for (int i = 0; i < ra->high_low_container.size; ++i){
        res_type[ra->high_low_container.typecodes[i]]++;
        res_num.push_back(container_get_cardinality(ra->high_low_container.containers[i],
                                          ra->high_low_container.typecodes[i]));
    }
}

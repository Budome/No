/* auto-generated on Mon Jul  6 11:06:22 CST 2020. Do not edit! */
#include "roaring.h"
...
class Roaring {
    ...
    */
    ...
    /**
    * get number of each kind of container
    */
    void each_container_type(std::map<int, uint64_t>& res_type, std::vector<int>& res_num){
        if (res_type.count(BITSET_CONTAINER_TYPE_CODE) == 0){
            res_type[BITSET_CONTAINER_TYPE_CODE] = 0;
            res_type[ARRAY_CONTAINER_TYPE_CODE] = 0;
            res_type[RUN_CONTAINER_TYPE_CODE] = 0;
            res_type[SHARED_CONTAINER_TYPE_CODE] = 0;
        }
        roaring_bitmap_get_each_container_type(&roaring, res_type, res_num);
    }
...
class Roaring64Map {
    ...
    */
    void each_container_type(std::map<int, uint64_t>& res_type, std::vector<int>& res_num,
        ...
        res_type[RUN_CONTAINER_TYPE_CODE] = 0;
        res_type[SHARED_CONTAINER_TYPE_CODE] = 0;
        hb_num = roarings.size();
        for (auto &container : roarings){
            container.second.each_container_type(res_type, res_num);
        }
    }
   ...
   private:
    std::unordered_map<uint32_t, Roaring> roarings;
...
class Roaring64MapSetBitForwardIterator {
  ...
  { }
   ...
   protected:
	const std::unordered_map<uint32_t, Roaring>& p;
    std::unordered_map<uint32_t, Roaring>::const_iterator map_iter;
    std::unordered_map<uint32_t, Roaring>::const_iterator map_end;
    roaring_uint32_iterator_t i;
};
...
class Roaring64MapSetBitBiDirectionalIterator final :public Roaring64MapSetBitForwardIterator {
 ...
 protected:
	std::unordered_map<uint32_t, Roaring>::const_iterator map_begin;
};
